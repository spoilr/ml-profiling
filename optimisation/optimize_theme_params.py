...
def cross_validation(known_dataset, known_targets, fusion_algorithm, ids, current_svm):
	...
	for train_index, test_index in kf:
		error, f1, mis_ids, (hp, hr, hf), (cp, cr, cf) = fusion_outputs(known_dataset, known_targets, train_index, test_index, fusion_algorithm, ids, current_svm)
...
if __name__ == "__main__":
	...
	for pair in itertools.product(itertools.product(C_range,C_range), itertools.product(gamma_range,gamma_range)):
		...
		f1_wmaj, error_wmaj, wmhf, wmcf = cross_validation(dataset, targets, 'wmaj', ids, current_svm)
		
		if f1_wmaj == 0:
			continue
		
		if wmhf >= 0.4 and wmcf >= 0.4:
